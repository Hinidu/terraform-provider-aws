...
func resourceAwsGameliftMatchmakingConfigurationCreate(d *schema.ResourceData, meta interface{}) error {
	...
	d.SetId(*out.Configuration.ConfigurationArn)
...
func resourceAwsGameliftMatchmakingConfigurationRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		if isAWSErr(err, gamelift.ErrCodeInvalidRequestException, "Failed to find configuration") {
	...
	d.Set("creation_time", configuration.CreationTime)
	...
	d.Set("game_session_queue_arns", schema.NewSet(schema.HashString, flattenStringList(configuration.GameSessionQueueArns)))
